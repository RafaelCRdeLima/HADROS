...
{
    const double eps = 1.0e-5 * std::max(1.0, std::abs(r));

    double gp[4][4];
    double gm[4][4];

    metric_.inverse_metric(r + eps, th, gp);
    metric_.inverse_metric(r - eps, th, gm);

    return (gp[mu][nu] - gm[mu][nu]) / (2.0 * eps);
}

double KerrGeodesic::dg_inv_dtheta(
    int mu,
    int nu,
    double r,
    double th
) const
{
    ...
    metric_.inverse_metric(r, th + eps, gp);
    metric_.inverse_metric(r, th - eps, gm);
...
) const
{
    ...
    for (int mu = 0; mu < 4; ++mu) {
        for (int nu = 0; nu < 4; ++nu) {
            dydl.pr -= 0.5 *
                dg_inv_dr(mu, nu, y.r, y.theta)
                * p[mu] * p[nu];

            dydl.ptheta -= 0.5 *
                dg_inv_dtheta(mu, nu, y.r, y.theta)
                * p[mu] * p[nu];
        }
    }
