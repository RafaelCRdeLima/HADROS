...
# 3) Reuse that cache to compute tau, P_surv, UHE intensity,
#    MeV diagnostics, and the image data table.
...
# 4) Replot an already computed image without recomputing geodesics or tau.
#    Keep the same tags so the script finds the intended data file.
...
is unchanged. The output image tables are written to `output/images/`, and the
plots are written under `plots/rho0_torus_*/ENU*/`.
...
`kerr-criar-cache` runs the same chain in one target: generate geodesics,
compute the cached image/optical depths, and plot the result.
