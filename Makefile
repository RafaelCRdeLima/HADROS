...
kerr-criar-cache: dirs kerr-geodesics compute_kerr_image_from_cache
	...
	DENSITY_PROFILE=$(DENSITY_PROFILE) SOURCE_MODEL=$(SOURCE_MODEL) SPECTRAL_MODEL=$(SPECTRAL_MODEL) MEV_SPECTRAL_MODE=$(MEV_SPECTRAL_MODE) MEV_E_MIN_MEV=$(MEV_E_MIN_MEV) MEV_E_MAX_MEV=$(MEV_E_MAX_MEV) $(PYTHON) $(SCRIPT_DIR)/plot_kerr_image.py $(ENU) $(MEV_ENU) $(MEV_NORM) $(CAM_THETA_DEG) $(TORUS_RHO0)
...
image-from-small-cache: dirs compute_kerr_image_from_cache
	...
	DENSITY_PROFILE=$(DENSITY_PROFILE) SOURCE_MODEL=$(SOURCE_MODEL) SPECTRAL_MODEL=$(SPECTRAL_MODEL) MEV_SPECTRAL_MODE=$(MEV_SPECTRAL_MODE) MEV_E_MIN_MEV=$(MEV_E_MIN_MEV) MEV_E_MAX_MEV=$(MEV_E_MAX_MEV) $(PYTHON) $(SCRIPT_DIR)/plot_kerr_image.py $(SMALL_CACHE_ENU) $(MEV_ENU) $(MEV_NORM) $(CAM_THETA_DEG) $(SMALL_CACHE_TORUS_RHO0)
